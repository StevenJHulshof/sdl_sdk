...
void Tile::render(int xPos, int zPos) {
	renderGraphics(xPos, zPos);
}

void Tile::renderGraphics(int xPos, int zPos)
{
	int w = gTextures[_textureType].getWidth() * _zoom;
	int h = gTextures[_textureType].getHeight() * _zoom;

	Translate::gridToScreenPos(xPos, zPos, w, h, &_screenPosX, &_screenPosY);
